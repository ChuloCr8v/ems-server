  ...
  async updateUser(
    ...
    if (data.departments?.length > 0) {
      ...
      if (data.departments.length > 1) {
        ...
        if (!data.departments.includes(data.defaultDepartment))
          bad('Select user assigned department');
  ...
  private async ensureUniqueEmployeeFields(
    ...
    if (workPhone) {
      ...
      if (existingWorkPhone)
        bad(
          `Work phone "${workPhone}" is already assigned to another employee`,
        );
    }
  ...
  async addEmployee(data: AddEmployeeDto[]) {
    ...
    for (const e of data) {
      try {
        ...
        // Duplicate checks
        if (e.email) {
          const found = existingEmails.find(
            (u) => u.email.toLowerCase() === e.email.toLowerCase(),
          );
          if (found)
            throw new Error(
              `Email ${e.email} already belongs to ${found.firstName} ${found.lastName}`,
            );
        }
        if (e.workPhone) {
          const found = existingWorkPhones.find(
            (u) => u.workPhone === e.workPhone.toString(),
          );
          if (found)
            throw new Error(
              `Work phone ${e.workPhone} already belongs to ${found.firstName} ${found.lastName}`,
            );
        }
        if (e.phone) {
          const found = existingPhones.find(
            (u) => u.phone === e.phone.toString(),
          );
          if (found)
            throw new Error(
              `Phone ${e.phone} already belongs to ${found.firstName} ${found.lastName}`,
            );
        }
        if (e.eId) {
          ...
          if (found)
            throw new Error(
              `Employee ID ${e.eId} already belongs to ${found.firstName} ${found.lastName}`,
            );
        }
        ...
        if (departments.length !== e.department.length) {
          throw new Error(
            `Some departments not found: expected ${e.department.length}, found ${departments.length}`,
          );
        }
    ...
    const failed = results
      .filter((r) => !r.success)
      .map((r) => ({ error: r.error, input: r.input }));
