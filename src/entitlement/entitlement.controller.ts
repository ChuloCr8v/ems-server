...
export class EntitlementController {
  ...
  async createEntitlement(@Body() dto: EntitlementDto, @Res() res: Response) {
    ...
    return res
      .status(200)
      .json({ message: `An Entitlement Has Been Created`, entitlement });
  }
  ...
  async getEmployeeLeaveEntitlement(@Req() req: ReqPayload) {
    return await this.entitlement.getEmployeeLeaveEntitlement(
      req.user.id,
      'LEAVE',
    );
  }
  ...
  async updateEntitlement(
    ...
    return res
      .status(200)
      .json({ message: `An Entitlement Has Been Updated`, entitlement });
  }
  ...
  async deleteEntitlement(@Param('id') id: string, @Res() res: Response) {
    ...
    return res
      .status(200)
      .json({ message: `An Entitlement Has Been Deleted`, entitlement });
  }
}
