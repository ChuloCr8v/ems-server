...
export class DepartmentController {
  ...
  @Auth()
  @Get('/:deptId/teams')
  async listTeams(@AuthUser() req: IAuthUser, @Param('deptId') deptId: string) {
    return await this.departmentService.listTeams(deptId);
  }

  @Auth()
  @Get('/:deptId/team')
  async getTeam(@AuthUser() req: IAuthUser, @Param('deptId') teamId: string) {
  ...
  async updateDepartment(
    @Param('id') id: string,
    @Body() update: Partial<DepartmentDto>,
    @Res() res: Response,
  ) {
    ...
    return res
      .status(200)
      .json({ message: `Department Has Been Updated`, department });
  }
  ...
  async addDepartmentMembers(
    ...
    const department = await this.departmentService.addDepartmentMembers(
      deptId,
      userIds,
    );
    return res.status(200).json({
      message: `Team members has been successfully added`,
      department,
    });
  }
  ...
  async addTeamMembers(
    ...
    const department = await this.departmentService.addTeam(
      deptId,
      createdById,
      data,
    );
    return res
      .status(200)
      .json({ message: `Team has created successfully`, department });
  }
  ...
  async updateTeam(
    ...
    return res
      .status(200)
      .json({ message: `Team has been updated successfully`, department });
  }
  ...
  async deleteDepartment(@Param('id') id: string, @Res() res: Response) {
    ...
    return res
      .status(200)
      .json({ message: `Department has been deleted successfully` });
  }
  ...
  async deleteTeam(@Param('id') id: string, @Res() res: Response) {
    ...
    return res
      .status(200)
      .json({ message: `Team has been deleted successfully` });
  }
}
