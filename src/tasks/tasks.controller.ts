...
export class TasksController {
  ...
  @Auth()
  @Get('category')
  listTaskCategories(@Req() req: ReqPayload) {
    return this.tasksService.listTaskCategories(req.user.id);
  }

  @Auth()
  @Get(':id/category')
  getTaskCategory(@Param('id') id: string) {
  ...
  @Auth()
  @Get()
  getAllTasks(@Req() req: ReqPayload) {
  ...
  @Auth()
  @Delete(':id')
  async deleteTask(@Param('id') id: string, @AuthUser() req: IAuthUser) {
  ...
  async transferTask(
    @Param('id') id: string,
    @Req() req: ReqPayload,
    @Body() dto: { ownerId: string; newDeliveryDate: Date; note?: string },
  ) {
    return this.tasksService.transfer(
      id,
      req.user.id,
      dto.ownerId,
      dto.newDeliveryDate,
  ...
  async extendDueDate(
    @Param('id') id: string,
    @Req() req: ReqPayload,
    @Body() dto: { newDeliveryDate: Date; note?: string },
  ) {
    return this.tasksService.extendDueDate(
      id,
      req.user.id,
      dto.newDeliveryDate,
      dto.note,
  ...
  async requestExtension(
    ...
    return this.tasksService.requestExtension(
      id,
      req.user.id,
      dto.newDeliveryDate,
      dto.note,
  ...
  async hideExtension(@Param('id') id: string) {
