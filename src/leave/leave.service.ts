...
export class LeaveService {
  ...
  async checkLeaveBalance(userId: string, typeId: string) {
    try {
      ...
      throw new BadRequestException(
        'Failed to check leave balance: ' + error.message,
      );
    }
  }
  ...
  async initializeApprovalFlow(
    ...
    if (approvers.length === 0) {
      throw new NotFoundException(
        'No valid approvers found (cannot self-approve)',
      );
    }
    ...
    for (const approver of approvers) {
      const canApprove = await this.approver.canUserApprove(
        approver.id,
        userId,
      );
  ...
  async rejectLeaveRequest(
    ...
    try {
      ...
      return this.prisma.$transaction(async (tx) => {
        ...
        // setTimeout(() => {
        this.sendRejectionMail(approval.leaveRequestId, note).catch(
          console.error,
        );
        // }, 0);
  ...
  async deleteLeaveRequest(leaveRequestId: string, userId: string) {
    try {
      ...
      if (!user || user.id !== leaveReq.userId)
        mustHave(user, 'Unauthorized', 404);

      if (leaveReq.status === LeaveStatus.APPROVED)
        bad("You can't delete an approved request");
  ...
  async cancelLeaveRequest(leaveRequestId: string, userId: string) {
    try {
      ...
      if (leaveReq.status === LeaveStatus.APPROVED)
        bad("You can't cancel an approved request");
