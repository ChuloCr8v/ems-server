...
export class PipService {
  ...
  async createPip(userId: string, data: CreatePipDto) {
    try {
      ...
      const createdPip = await this.prisma.pip.create({
        data: {
      ...
      // Notify department manager(s) about the new PIP
      const department = await this.prisma.department.findUnique({
        where: { id: user.departments[0].id },
        include: {
          approver: {
            where: { role: Role.DEPT_MANAGER },
            select: { userId: true },
          },
        },
      });

      const managerIds = department?.approver?.map((a) => a.userId) ?? [];

      if (managerIds.length > 0) {
        this.eventEmitter.emit(
          'pip.created',
          new PipCreatedEvent(createdPip.id, user.id, managerIds),
        );
      }
  ...
  async getMyPips(userId: string) {
    try {
      ...
      if (isManager) {
        ...
        return await this.prisma.pip.findMany({
          where: {
            OR: [{ userId: user.id }, { userId: { in: memberIds } }],
          },
          include: includeOpts,
        });
      }
  ...
  async recommendPip(userId: string, data: RecommendPipDto) {
    try {
      ...
      //DEPT_MANGERS: can only recommend for users in their department
        ...
        targetUser = department.user.find(
  ...
  async approvePip(userId: string, pipId: string, data: ApprovePipDto) {
    try {
      ...
      if (ownerIsManager) {
        ...
        this.eventEmitter.emit(
          'pip.ManagerApproved',
          new PipApprovedEvent(approvedPip.id, user.id, pip.userId, [
            pip.userId,
          ]),
        );
  ...
  async sendToHr(userId: string, departmentId: string) {
    try {
      ...
      const department_Manager = department.approver.find(
        (approver) =>
          approver.userId === user.id && approver.role === 'DEPT_MANAGER',
      );
      if (!department_Manager)
        throw bad('You are not the manager of this department');
      ...
      await this.prisma.department.update({
        ...
        data: { pipStatus: 'MANAGER_APPROVED' },
      });
  ...
  async rejectPip(userId: string, pipId: string, data: RejectPipDto) {
    ...
    if (ownerIsManager) {
      ...
      const { rejectedPip, rPip } = await this.prisma.$transaction(
        async (tx) => {
          //Reject PIP
  ...
  async getAllManagersPip(userId: string) {
    try {
      ...
      const isSuperAdmin = this.userHasRole(user, Role.SUPERADMIN);
      if (!isSuperAdmin)
        throw bad('Only Super Admin can view all managers PIPs');
  ...
  async makePipClaimRequest(pipId: string, userId: string) {
    try {
      ...
      if (pip.userId !== user.id)
        throw bad('You can only make claim request for your own PIP');
      if (pip.status !== 'COMPLETED')
        throw bad('Only completed PIPs are eligible for claim requests');
      ...
      const claim = await this.prisma.claim.create({
        data: {
          ...
          proofUrls:
            pip.uploads && pip.uploads.length > 0
              ? {
