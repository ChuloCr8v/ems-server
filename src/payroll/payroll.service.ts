...
} from 'src/constants/static-components';
import {
  AddComponentDto,
  PayrollDto,
  UpdatePayrollDto,
...
export class PayrollService {
  ...
  async calculatePayRoll(data: PayrollDto) {
    try {
      ...
      const { gross, deductions, components } = this.calculatePayrollComponents(
        salary,
        data.components,
      );
  ...
  async removeCustomComponent(id: string) {
    try {
      ...
      if (
        component.category !== ComponentCategory.CUSTOM_EARNING &&
      ...
      throw new BadRequestException(
        'Failed to remove custom component:' + error.message,
      );
    }
  }
  ...
  async findAllPayroll() {
    try {
      ...
      throw new BadRequestException(
        'Failed to find all payroll:' + error.message,
      );
    }
  }
  ...
  async findOnePayroll(payrollId: string) {
    try {
      ...
      throw new BadRequestException(
        'Failed to find one payroll:' + error.message,
      );
    }
  }
  ...
  private async generateConsolidatedDeductionSummary(
    payrolls: { id: string }[],
  ): Promise<void> {
    const now = new Date();
    const year = now.getFullYear();
    const monthInWords = now.toLocaleDateString('en-US', {
      month: 'long',
      year: 'numeric',
    });
    ...
    for (const c of comps) {
      ...
      if (c.title === 'Pension Contribution')
        emp.employeePension = c.monthlyAmount || 0;

      emp.totalPension =
        (emp.employeePension || 0) + (emp.employerPension || 0);
    }
  ...
  async downloadDeductionsExcel(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${deduction.name}.xlsx"`,
    );
  ...
  private calculateComponentAmounts = (
    ...
    return component.map((comp) => {
      ...
      if (comp.calculations === SalaryCalculationType.PERCENTAGE) {
        ...
        const custom =
          comp.category === ComponentCategory.CUSTOM_DEDUCTION ||
          comp.category === ComponentCategory.CUSTOM_EARNING;
  ...
  private async recalculatePayrollTotals(payrollId: string) {
    try {
      ...
      const taxResults = await this.calculateNigerianTaxes(
        payroll.component,
        payroll.gross,
      );
      ...
      for (const component of payroll.component) {
        if (component.type === SalaryType.EARNING) {
          ...
          if (
            !['Pension Contribution', 'NHF Contribution', 'PAYE Tax'].includes(
              component.title,
            )
          ) {
            deductions += component.annualAmount;
      ...
      throw new BadRequestException(
        'Failed to recalculate payroll totals:' + error.message,
      );
    }
  }
  ...
  private async updateTaxComponents(payrollId: string, taxResults: any) {
    try {
      ...
      for (const taxComp of taxComponents) {
        const existingComponent = payroll.component.find(
          (comp) => comp.title === taxComp.title,
        );
      ...
      throw new BadRequestException(
        'Failed to update tax components:' + error.message,
      );
    }
  }
  ...
  private async calculateNigerianTaxes(components: any[], gross: number) {
    try {
      ...
      const transportComponent = components.find(
        (c) => c.title === 'Transport',
      );

      //Extract life assurance component if exists
      const lifeAssuranceComponent = components.find(
        (c) =>
      ...
      throw new BadRequestException(
        'Failed to calculate nigerian taxes:' + error.message,
      );
    }
  }
  ...
  private async generateDeductionsExcel({
    ...
    try {
      ...
      [5, 6, 7, 8].forEach((colIndex) => {
        // Columns E and F (5th and 6th columns)
